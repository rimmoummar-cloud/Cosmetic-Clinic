...
export default function AdminCalendarPage() {
  ...
  const fetchCustomerNotes = async (customerId) => {
    ...
    try {
      ...
      const response = await api.get(`/note/customer/${customerId}`);
  ...
  const handleOpenNotes = async () => {
    const customerId = selectedBooking?.customer_id || selectedBooking?.customer?.id;

    if (!customerId) {
      setNoteError("Customer ID not found for this booking.");
      setNotesOpen(true);
      return;
    }
  ...
  const handleSaveNote = async () => {
    ...
    try {
      ...
      if (editingNoteId) {
        const response = await api.put(`/note/${editingNoteId}`, {
          note: noteText.trim(),
        });
        ...
        const response = await api.post(`/note/customer/${customerId}`, {
          note: noteText.trim(),
        });

        setCustomerNotes((currentNotes) => [response.data, ...currentNotes]);
      }
  ...
  const handleDeleteNote = async (noteId) => {
    try {
      ...
      setCustomerNotes((currentNotes) =>
        currentNotes.filter((note) => String(note.id) !== String(noteId))
      );
            ...
            <div className="min-h-0 flex-1 overflow-y-auto p-6">
              <div className="rounded-2xl border border-gray-200 bg-gray-50 p-4">
                  ...
                  <div>
                    ...
                    {editingNoteId && (
                      <p className="mt-1 text-xs text-gray-500">
                        Update the existing customer note.
                      </p>
                    )}
                  </div>
                ...
                <textarea
                  ...
                  rows={4}
                  className="w-full resize-none rounded-xl border border-gray-200 bg-white px-3 py-3 text-sm text-gray-700 outline-none transition placeholder:text-gray-400 focus:border-primary focus:ring-2 focus:ring-primary/10"
                />
              ...
              <div className="mt-6">
                ...
                {notesLoading ? (
                  <div className="flex min-h-[160px] items-center justify-center rounded-2xl border border-dashed border-gray-200 text-sm text-gray-500">
                    Loading notes...
                  </div>
                        ...
                        <div className="flex items-start justify-between gap-4">
                          ...
                          <div className="flex shrink-0 items-center gap-2">
                            ...
                            <button
                              ...
                              className="rounded-lg border border-red-100 px-2.5 py-1.5 text-xs font-medium text-red-600 transition hover:bg-red-50 disabled:cursor-not-allowed disabled:opacity-50"
                            >
                  ...
                  <div className="flex min-h-[160px] items-center justify-center rounded-2xl border border-dashed border-gray-200 bg-gray-50 text-sm text-gray-500">
