...
//   const [builderTemplate, setBuilderTemplate] = useState({ ...emptyTemplateForm });
...
//   const { data: completedForms = [], isLoading: completedFormsLoading } = useQuery({
...
//   const { data: selectedTemplateDetails, isLoading: templateDetailsLoading, refetch: refetchTemplateDetails } = useQuery({
//     queryKey: ["form-template-details", selectedTemplateId],
//     queryFn: async () => {
//       if (!selectedTemplateId) return null;
//       const res = await api.get(`/forms/templates/${selectedTemplateId}/with-details`);
//       return res.data;
//     },
//     enabled: !!selectedTemplateId,
//   });

//   const { data: selectedCompletedFormDetails, isLoading: completedFormDetailsLoading } = useQuery({
//     queryKey: ["completed-form-details", selectedTemplateId],
//     queryFn: async () => {
//       if (!selectedTemplateId) return null;
//       const res = await api.get(`/forms/completed-forms/${selectedTemplateId}/with-details`);
//       return res.data;
//     },
//     enabled: false,
//   });
...
//     try {
//       const res = await api.get(`/forms/templates/${template.id}/with-details`);
...
//       });
//       setBuilderQuestions((details.questions || []).map((question) => normalizeQuestion(question)));
//       setRemovedQuestionIds([]);
//       setRemovedOptionIds([]);
//       setBuilderOpen(true);
//     } catch (error) {
...
//         alert("Form name is required before adding an option.");
//         return null;
//       }

//       const createdTemplate = await api.post("/forms/templates", {
//         name: builderTemplate.name.trim(),
//         description: builderTemplate.description.trim(),
//       });
//       templateId = createdTemplate.data?.id || createdTemplate.data?.template?.id;
//       setBuilderTemplate((current) => ({ ...current, id: templateId }));
//       setBuilderMode("edit");
//     }
...
//       alert("Question text is required before adding an option.");
//       return null;
//     }

//     const createdQuestion = await api.post("/forms/questions", {
...
//     setBuilderQuestions((current) =>
//       current.map((item, index) => (index === questionIndex ? { ...item, id: questionId } : item))
//     );
...
//     try {
//       const questionId = await ensureQuestionIsPersisted(questionIndex);
...
//             options: [
//               ...question.options,
//               { id: null, option_label: "", sort_order: question.options.length },
//             ],
//           };
//         })
//       );
//     } catch (error) {
//       console.error("Failed to prepare question for an option", error);
//       alert(error?.response?.data?.error || "Failed to prepare question for an option");
//     }
//   };

//   const saveOption = async (questionIndex, optionIndex) => {
//     const question = builderQuestions[questionIndex];
//     const option = question?.options?.[optionIndex];
//     if (!question || !option || option.id || !option.option_label.trim()) return;

//     try {
//       const questionId = await ensureQuestionIsPersisted(questionIndex);
//       if (!questionId) return;

//       const response = await api.post("/forms/options", {
...
//           return {
//             ...item,
//             options: item.options.map((itemOption, indexInQuestion) =>
//               indexInQuestion === optionIndex
//                 ? {
//                     id: createdOption?.id,
//                     option_label: createdOption?.option_label || option.option_label.trim(),
//                     sort_order: createdOption?.sort_order ?? option.sort_order,
//                   }
//                 : itemOption
//             ),
//           };
...
//       );
//     } catch (error) {
//       console.error("Failed to create form option", error);
//       alert(error?.response?.data?.error || "Failed to create form option");
//     }
//   };

//   const handleOptionKeyDown = (event, questionIndex, optionIndex) => {
//     if (event.key === "Enter") {
...
//       const cleanQuestions = builderQuestions.map((question, index) => ({
//         ...question,
//         question_text: question.question_text.trim(),
//         required: Boolean(question.required),
//         sort_order: index,
...
//       const invalidQuestions = cleanQuestions.filter(
//         (question) => !question.question_text
//       );

//       if (invalidQuestions.length > 0) {
//         alert("Each question must have text before saving.");
//         return;
//       }
...
//         const templateId = createdTemplate.data?.id || createdTemplate.data?.template?.id;
...
//         await api.put(`/forms/templates/${templateId}`, {
//           name: builderTemplate.name.trim(),
//           description: builderTemplate.description.trim(),
//           is_active: builderTemplate.is_active,
//         });

//         const originalQuestionIds = new Set(
//           (selectedTemplateDetails?.questions || []).map((question) => question.id)
//         );
//         const currentQuestionIds = new Set(
//           cleanQuestions.filter((question) => question.id).map((question) => question.id)
//         );
...
//         for (const question of cleanQuestions) {
//           if (question.id) {
//             await api.put(`/forms/questions/${question.id}`, {
//               question_text: question.question_text,
//               question_type: question.question_type,
//               required: question.required,
//               sort_order: question.sort_order,
...
//               sort_order: question.sort_order,
//             });
//             question.id = createdQuestion.data?.id;
//           }

//           const existingQuestionOptions =
//             (selectedTemplateDetails?.questions || []).find((item) => item.id === question.id)?.options || [];
//           const questionOptionIds = new Set(
//             existingQuestionOptions.filter((option) => option.id).map((option) => option.id)
//           );
//           const currentOptionIds = new Set(
//             (question.options || []).filter((option) => option.id).map((option) => option.id)
//           );
...
//             if (!option.option_label) continue;
//             if (option.id) {
//               await api.put(`/forms/options/${option.id}`, {
//                 option_label: option.option_label,
//                 sort_order: option.sort_order,
//               });
//             } else {
...
//         queryClient.invalidateQueries({ queryKey: ["forms-completed"] }),
//       ]);
...
//       alert(error?.response?.data?.error || "Failed to save form template");
//     }
//   };
...
//     try {
//       await api.delete(`/forms/templates/${template.id}`);
//       if (selectedTemplateId === template.id) {
//         setSelectedTemplateId(null);
//       }
//       await queryClient.invalidateQueries({ queryKey: ["forms-templates"] });
//     } catch (error) {
...
//     if (!template?.id) return;
//     try {
//       await api.patch(`/forms/templates/${template.id}/deactivate`);
//       await queryClient.invalidateQueries({ queryKey: ["forms-templates"] });
//     } catch (error) {
//       console.error("Failed to update template state", error);
//       alert(error?.response?.data?.error || "Failed to update template state");
//     }
//   };

//   const selectedCompletedFormId = selectedTemplateId;
//   const displayedCompletedForm = selectedCompletedFormDetails;

//   return (
//     <div className="space-y-6">
//       <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
//         <div>
//           <h1 className="text-2xl font-bold font-[var(--font-heading)] text-gray-900">Forms</h1>
//           <p className="mt-1 text-sm text-gray-500">Manage form templates and historical completed submissions.</p>
//         </div>
...
//                       <p className="mt-1 text-sm text-gray-500">{template.description || "No description provided."}</p>
//                     </div>
//                     <span
...
//               <h2 className="text-lg font-semibold text-gray-900">Completed Forms</h2>
//             </div>

//             {completedFormsLoading ? (
//               <div className="p-6 text-sm text-gray-500">Loading completed forms...</div>
//             ) : completedForms.length === 0 ? (
//               <div className="p-8 text-center text-sm text-gray-500">No completed forms found.</div>
//             ) : (
...
//                       <tr key={form.id} className="border-b border-gray-100 last:border-0">
//                         <td className="px-4 py-3">
//                           <div className="font-medium text-gray-900">{form.form_name || `Form #${form.id}`}</div>
//                           <div className="text-xs text-gray-500">#{form.id}</div>
//                         </td>
//                         <td className="px-4 py-3 text-gray-700">{form.customer_name || form.customer_id || "-"}</td>
//                         <td className="px-4 py-3">
//                           <span
...
//                           </span>
//                         </td>
//                         <td className="px-4 py-3 text-gray-600">{formatDate(form.created_at)}</td>
//                         <td className="px-4 py-3 text-right">
//                           <button
//                             type="button"
//                             onClick={async () => {
//                               try {
//                                 const res = await api.get(`/forms/completed-forms/${form.id}/with-details`);
//                                 const details = res.data;
...
//                                 setBuilderTemplate({
//                                   id: details.id,
//                                   name: details.form_name || "Completed form",
//                                   description: details.description || "",
//                                   is_active: true,
//                                 });
//                                 setBuilderQuestions((details.questions || []).map((question) => ({
...
//           <div className="rounded-2xl border border-gray-200 bg-white shadow-sm">
//             <div className="border-b border-gray-200 px-4 py-4">
//               <h2 className="text-lg font-semibold text-gray-900">Historical Form Details</h2>
//             </div>

//             {!displayedCompletedForm ? (
//               <div className="p-8 text-center text-sm text-gray-500">Select a completed form to inspect the original answers.</div>
//             ) : (
//               <div className="space-y-5 p-5">
//                 <div className="rounded-2xl bg-gray-50 p-4">
//                   <p className="text-xs uppercase tracking-[0.16em] text-gray-400">Customer</p>
//                   <h3 className="mt-2 text-xl font-bold text-gray-900">
//                     {displayedCompletedForm.customer?.name || "Customer"}
//                   </h3>
//                   <p className="text-sm text-gray-600">{displayedCompletedForm.customer?.email || "-"}</p>
//                   <p className="text-sm text-gray-600">{displayedCompletedForm.customer?.phone || "-"}</p>
//                 </div>

//                 <div className="rounded-2xl border border-gray-200 bg-white p-4">
//                   <p className="text-xs uppercase tracking-[0.16em] text-gray-400">Form</p>
//                   <p className="mt-2 text-lg font-semibold text-gray-900">
//                     {displayedCompletedForm.form_name || "Completed form"}
//                   </p>
//                   <p className="mt-1 text-sm text-gray-500">Status: {displayedCompletedForm.status}</p>
//                 </div>

//                 <div className="space-y-4">
//                   {(displayedCompletedForm.questions || []).map((question) => {
...
//                             question.options?.find((option) => String(option.id) === String(answer.selected_option_id))?.option_label || "No option selected"
//                           ) : question.question_type === "yes_no" || question.question_type === "agreement" ? (
//                             answer.boolean_value === true ? "Yes" : answer.boolean_value === false ? "No" : "Not answered"
//                           ) : (
...
//               <div>
//                 <p className="text-xs font-semibold uppercase tracking-[0.18em] text-gray-400">Form builder</p>
//                 <h2 className="mt-1 text-2xl font-bold text-gray-900">
//                   {builderMode === "create" ? "Create Form" : builderMode === "view" ? "Completed Form" : "Edit Form"}
//                 </h2>
//               </div>
//               <button
//                 type="button"
...
//                   <div key={question.id || `view-${question.question_text}`} className="rounded-2xl border border-gray-200 p-4">
//                     <p className="font-medium text-gray-900">{question.question_text}</p>
//                     <div className="mt-3 rounded-xl bg-gray-50 p-3 text-sm text-gray-700">
//                       {question.question_type === "multiple_choice" ? (
//                         question.options?.find((option) => String(option.id) === String(question.answer?.selected_option_id))?.option_label || "No option selected"
//                       ) : question.question_type === "yes_no" || question.question_type === "agreement" ? (
//                         question.answer?.boolean_value === true ? "Yes" : question.answer?.boolean_value === false ? "No" : "Not answered"
//                       ) : (
//                         question.answer?.answer_text || "No response recorded"
//                       )}
//                     </div>
//                   </div>
//                 ))}
...
//                       onChange={(event) => setBuilderTemplate({ ...builderTemplate, name: event.target.value })}
...
//                       onChange={(event) =>
//                         setBuilderTemplate({ ...builderTemplate, description: event.target.value })
//                       }
//                       rows={3}
//                       className="w-full rounded-xl border border-gray-200 bg-white px-3 py-2.5 text-sm outline-none focus:border-primary"
...
//                     <input
//                       type="checkbox"
//                       checked={builderTemplate.is_active !== false}
//                       onChange={(event) =>
//                         setBuilderTemplate({ ...builderTemplate, is_active: event.target.checked })
//                       }
//                     />
//                     Active template
//                   </label>
...
//                         No questions yet. Start building the form.
//                       </div>
//                     ) : (
//                       builderQuestions.map((question, questionIndex) => (
//                         <div key={`${question.id || "new"}-${questionIndex}`} className="rounded-2xl border border-gray-200 p-4">
//                           <div className="flex items-start justify-between gap-3">
//                             <div className="min-w-0 flex-1 space-y-3">
//                               <input
//                                 value={question.question_text}
//                                 onChange={(event) =>
//                                   handleQuestionChange(questionIndex, "question_text", event.target.value)
//                                 }
...
//                                   onChange={(event) =>
//                                     handleQuestionChange(questionIndex, "question_type", event.target.value)
//                                   }
//                                   className="w-full rounded-xl border border-gray-200 px-3 py-2 text-sm outline-none focus:border-primary"
//                                 >
...
//                                     onChange={(event) =>
//                                       handleQuestionChange(questionIndex, "required", event.target.checked)
//                                     }
//                                   />
//                                   Required
//                                 </label>
...
//                                     Add one or more choices for this multiple-choice question.
//                                   </div>
//                                 ) : (
...
//                                           )
//                                         }
//                                         onBlur={() => saveOption(questionIndex, optionIndex)}
//                                         onKeyDown={(event) =>
//                                           handleOptionKeyDown(event, questionIndex, optionIndex)
//                                         }
...
//                   >
//                     {builderMode === "create" ? "Done" : "Save Changes"}
//                   </button>
//                 </div>
//               </div>
//             )}
...
//     if (!form?.id) return;
//     if (!window.confirm("Delete this completed form?")) return;

//     try {
//       await api.delete(`/forms/completed-forms/${form.id}`);
//       await queryClient.invalidateQueries({ queryKey: ["forms-completed"] });
//     } catch (error) {
//       console.error("Failed to delete completed form", error);
//       alert(error?.response?.data?.error || "Failed to delete completed form");
//     }
//   };
...
//                           <button
//                             type="button"
//                             onClick={() => deleteCompletedForm(form)}
//                             className="rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-xs font-medium text-red-700 hover:bg-red-100"
//                           >
//                             Delete
//                           </button>
