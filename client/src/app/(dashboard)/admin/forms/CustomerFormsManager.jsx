...
export default function CustomerFormsManager({
  ...
  // const buildDuplicatedAnswerMap = (originalQuestions, newQuestions) => {
  //   const map = {};
  ...
  //     if (!originalQuestion) {
  //       map[newQuestion.id] = {
  //         completed_form_question_id: newQuestion.id,
  //         answer_text: null,
  ...
  //     if (newQuestion.question_type === "confirmation") {
  //       map[newQuestion.id] = {
  //         completed_form_question_id: newQuestion.id,
  //         answer_text: null,
  //         selected_option_id: null,
  ...
  //       completed_form_question_id: newQuestion.id,
  //       answer_text:
  //         originalQuestion.answer?.answer_text ?? null,
  //       selected_option_id:
  //         originalQuestion.answer?.selected_option_id ?? null,
  //       boolean_value: normalizeBoolean(
  //         originalQuestion.answer?.boolean_value
  //       ),
  //     };
  //   });
...
const buildDuplicatedAnswerMap = (originalQuestions, newQuestions) => {
  ...
  (newQuestions || []).forEach((newQuestion, index) => {
    ...
    if (newQuestion.question_type === "multiple_choice") {
      const oldSelectedOptionId =
        originalQuestion.answer?.selected_option_id;

      const oldSelectedOption = (
        originalQuestion.options || []
      ).find(
        (option) =>
          String(option.id) === String(oldSelectedOptionId)
      );
      ...
      if (oldSelectedOption) {
        const newOption = (
          ...
          (option) =>
            String(option.original_option_id) ===
            String(oldSelectedOption.original_option_id)
        );

        selectedOptionId = newOption?.id ?? null;
      }
    }
  ...
  const duplicateCompletedForm = async () => {
    if (!selectedCompletedForm) return;

    const shouldContinue = window.confirm(
      "Warning: The form template may have been changed since this customer's original form was created.\n\n" +
      "If questions or options were added, deleted, or changed, the duplicated form may not contain all of the original answers.\n\n" +
      "Answers can only be copied for questions and options that still match the original form. Any newly added questions will remain unanswered and must be completed manually.\n\n" +
      "Do you want to continue with the duplication?"
    );
    ...
    try {
      ...
      if (!resolvedCustomerId) {
        alert(
          "Cannot duplicate this form because the selected booking does not contain a valid customer ID."
        );
        return;
      }

      const templateId =
        selectedCompletedForm?.form_template_id ||
        selectedCompletedForm?.form_template?.id ||
        selectedCompletedForm?.template?.id;

      if (!isValidId(templateId)) {
        alert(
          "Cannot duplicate this form because the form template ID is missing."
        );
        return;
      }
      ...
      const newFormId = created?.data?.id;

      if (!newFormId) {
        throw new Error("The duplicated completed form ID was not returned.");
      }

      const detailsRes = await api.get(
        `/forms/completed-forms/${newFormId}/with-details`
      );

      const duplicatedDetails = detailsRes.data;
      const duplicatedQuestions =
        duplicatedDetails?.questions || [];

      setDraftFormId(newFormId);

      setSelectedTemplate({
        id: templateId,
        name:
          duplicatedDetails?.form_name ||
      ...
      setAnswers(
        buildDuplicatedAnswerMap(
          originalQuestions,
          duplicatedQuestions
        )
      );

      // Signature must always be entered again on the duplicated form.
      clearSignature();

      setSelectedCompletedFormId(newFormId);
      setSelectedCompletedForm(duplicatedDetails);

      setTab("create");
    } catch (error) {
      console.error("Failed to duplicate completed form", error);

      alert(
        error?.response?.data?.error ||
          error?.response?.data?.message ||
          "Failed to duplicate completed form"
      );
    } finally {
  ...
  const submitCurrentForm = async (saveAsDraft = false) => {
    ...
    //   [];
const payload = selectedTemplate?.questions || [];
  ...
  const renderQuestionEditor = (question) => {
    ...
    if (type === "confirmation") {
      return (
        <label className="mt-3 flex cursor-pointer items-start gap-3 rounded-xl border border-gray-200 bg-gray-50 p-3 text-sm text-gray-700">
          <input
            type="checkbox"
            checked={questionAnswer.boolean_value === true}
            onChange={(event) =>
              updateAnswer(
                question.id,
                ...
                <div className="min-w-0 rounded-2xl border border-gray-200 bg-white p-3 sm:p-4">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    {selectedCompletedForm && (
                      <button
                        type="button"
                        onClick={duplicateCompletedForm}
                        disabled={duplicating || saving}
                        className="shrink-0 rounded-xl bg-emerald-600 px-3 py-2 text-xs font-semibold text-white transition hover:bg-emerald-700 disabled:cursor-not-allowed disabled:opacity-60 sm:px-4 sm:text-sm"
                      >
