...
const resolveSnapshotQuestionId = (answers, questions, answer) => {
  ...
  if (answer.question_id) {
    const matchingQuestion = questions.find(
      (question) =>
        question.original_question_id === answer.question_id
    );
...
export const submitForm = async (req, res) => {
  ...
  try {
    ...
    if (!signatureValue) {
      ...
      return res.status(400).json({
        error:
          'Please add your customer signature before submitting the form.'
      });
    }
    ...
    if (completed_form_id) {
      completedForm =
        await CompletedForm.getCompletedFormById(
          client,
          completed_form_id
        );
      ...
      if (!completedForm) {
        ...
        return res.status(404).json({
          error: 'Completed form not found'
        });
      }
    } else {
      if (!customer_id || !form_template_id) {
        ...
        return res.status(400).json({
          error:
            'Missing required fields: customer_id, form_template_id'
        });
      }

      completedForm =
        await CompletedForm.createCompletedForm(client, {
          customer_id,
          appointment_id,
          form_template_id
        });
    ...
    const questions =
      await CompletedFormQuestion.getQuestionsByCompletedForm(
        client,
        completedForm.id
      );
    ...
    for (const answer of answers) {
      const completedFormQuestionId =
        resolveSnapshotQuestionId(
          answers,
          questions,
          answer
        );
      ...
      if (!completedFormQuestionId) {
        ...
        return res.status(400).json({
          error:
            'Each answer must include completed_form_question_id or a valid question_id'
        });
      }

      const snapshotQuestion =
        questionLookup.get(
          Number(completedFormQuestionId)
        );
      ...
      if (!snapshotQuestion) {
        ...
        return res.status(400).json({
          error: `Question ${completedFormQuestionId} does not belong to completed form ${completedForm.id}`
        });
      }
      ...
      if (answer.selected_option_id) {
        const optionRes = await client.query(
          `SELECT id FROM completed_form_question_options
           WHERE id = $1
             AND completed_form_question_id = $2`,
          [
            answer.selected_option_id,
            completedFormQuestionId
          ]
        ...
        if (!optionRes.rows[0]) {
          ...
          return res.status(400).json({
            error: `Option ${answer.selected_option_id} does not belong to question ${completedFormQuestionId}`
          });
        }
      }

      const existingAnswerRes =
        await client.query(
          `SELECT id FROM form_answers
        ...
        await client.query(
          `UPDATE form_answers
           SET answer_text = $3,
               ...
               updated_at = NOW()
           WHERE id = $1
             AND completed_form_id = $2`,
          [
            existingAnswerRes.rows[0].id,
            completedForm.id,
            answer.answer_text ?? null,
            answer.selected_option_id ?? null,
            answer.boolean_value ?? null
    ...
    const missingRequired = questions.filter(
      (question) =>
        question.required &&
        !answers.some((answer) => {
          const id = resolveSnapshotQuestionId(
            answers,
            questions,
            answer
          );
    ...
    if (missingRequired.length > 0) {
      ...
      return res.status(400).json({
        error: `Required question ${missingRequired[0].id} has no answer`
      });
    }
    ...
    const fullForm =
      await CompletedForm.getCompletedFormWithDetails(
        null,
        completedForm.id
      );
    ...
    return res.status(500).json({
      message:
        'Server error while submitting form',
      error: error.message
    });
  } finally {
    client.release();
...
export const saveDraft = async (req, res) => {
  ...
  try {
    ...
    await CompletedForm.updateCompletedFormStatus(
      client,
      completedForm.id,
      'draft'
    );
    ...
    return res.status(500).json({
      message:
        'Server error while saving form draft',
      error: error.message
    });
  } finally {
    client.release();
...
function validateAnswerForQuestionType(
  questionType,
  answerText,
  selectedOptionId,
  booleanValue
  ...
  switch (questionType) {
    ...
    case 'confirmation':
      if (
        ...
        throw new Error(
          `${questionType} questions require boolean_value (true/false)`
        );
      }
      break;
    ...
    case 'multiple_choice':
      if (!selectedOptionId) {
        throw new Error(
          'Multiple choice questions require selected_option_id'
        );
      }
      break;
