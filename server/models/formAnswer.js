...
export const getAnswersByCompletedForm = async (
  client = null,
  completedFormId
) => {
  const queryExecutor = client || db;
...
export const getAnswerWithQuestion = async (client = null, id) => {
  ...
  const result = await queryExecutor.query(
    `SELECT 
       ...
       cfq.question_text 
     FROM form_answers fa
     JOIN completed_form_questions cfq
       ON cfq.id = fa.completed_form_question_id
     WHERE fa.id = $1`,
    [id]
  );
...
export const createAnswer = async (client = null, data) => {
  ...
  if (!completed_form_id || !completed_form_question_id) {
    throw new Error(
      'Missing required fields: completed_form_id, completed_form_question_id'
    );
  }
  ...
  if (!formRes.rows[0]) {
    throw new Error(
      `Completed form with id ${completed_form_id} does not exist`
    );
  }
  ...
  if (!questionRes.rows[0]) {
    throw new Error(
      `Question with id ${completed_form_question_id} does not belong to completed form ${completed_form_id}`
    );
  }
  ...
  if (existingRes.rows[0]) {
    throw new Error(
      'An answer already exists for this completed form question'
    );
  }
  ...
  if (normalizedAnswer.selected_option_id !== null) {
    ...
    if (!optionRes.rows[0]) {
      throw new Error(
        `Option with id ${normalizedAnswer.selected_option_id} does not belong to question ${completed_form_question_id}`
      );
    }
  }
...
export const updateAnswer = async (client = null, id, data) => {
  ...
  const answerRes = await queryExecutor.query(
    `SELECT
       fa.completed_form_question_id,
       cfq.question_type,
       cfq.completed_form_id
     FROM form_answers fa
     JOIN completed_form_questions cfq
       ON cfq.id = fa.completed_form_question_id
     WHERE fa.id = $1`,
...
function validateAnswerForQuestionType(
  ...
  return normalizeAnswerForQuestionType(
    questionType,
    answerText,
    selectedOptionId,
    booleanValue
...
export const upsertAnswer = async (
  ...
  if (!questionRes.rows[0]) {
    throw new Error(
      `Question with id ${completedFormQuestionId} does not belong to completed form ${completedFormId}`
    );
  }
  ...
  if (normalizedAnswer.selected_option_id !== null) {
    ...
    if (!optionRes.rows[0]) {
      throw new Error(
        `Option with id ${normalizedAnswer.selected_option_id} does not belong to question ${completedFormQuestionId}`
      );
    }
  }
